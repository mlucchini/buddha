/**
* This is based MiniPID: https://github.com/tekdemo/MiniPID
* Gravity compensation feed-forward was added
*
...
double PID::getOutput(double actual, double setpoint) {
  ...
  //We ignore the error is it is within the dead band window
  if (abs(error) <= deadBandWindow) {
    error = 0;
  }
...
void PID::setDeadBandWindow(double window) {
  deadBandWindow = window;
}
